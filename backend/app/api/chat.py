...
async def chat_assistant(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    words = [w for w in re.split(r'[^a-zA-Z0-9]', question_lower) if len(w) > 2]
    ...
    for p in all_plants:
        ...
        if (
            ...
            or any(w in p.name.lower() for w in words)
            or any(w in p.botanical_name.lower() for w in words)
            or any(w in c_names for w in words)
            or any(w in full_text for w in words)
        ):
    ...
    if not matched_plants and all_plants:
        for p in all_plants:
            if is_clean_ascii_english(p.name) and p.id not in seen_ids:
                seen_ids.add(p.id)
                matched_plants.append({
                    "name": p.name,
                    "botanical_name": p.botanical_name,
